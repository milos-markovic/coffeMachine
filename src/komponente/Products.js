    ...
    const products = this.props.products.map(product => {
      return (
        <div className="col-md-3 mb-3 test" key={product.id}>
          <div className="card product" onClick={ () => {this.findProduct(product.id)} } >
            <img src={`/img/${product.img}`} alt="test" className="card-img-top product-img"/>
            <div className="card-header">
              <h6 className="card-title m-0 text-center">{product.name}</h6>
              <p className="m-0 text-center font-weight-bold">{product.price} din</p>
            </div>
          </div>
        </div>
      )
